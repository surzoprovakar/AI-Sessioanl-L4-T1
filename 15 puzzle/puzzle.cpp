...
double h2_eucledian(board b) {
    ...
    for(int i=0;i<4;i++) {
        for(int j=0;j<4;j++) {
            ...
            else if(b.brd[i][j]==3) ech = sqrt(pow((i-0),2)+pow((j-3),2));
            else if(b.brd[i][j]==4) ech = sqrt(pow((i-1),2)+pow((j-0),2));
            else if(b.brd[i][j]==5) ech = sqrt(pow((i-1),2)+pow((j-1),2));
            else if(b.brd[i][j]==6) ech = sqrt(pow((i-1),2)+pow((j-2),2));
            else if(b.brd[i][j]==7) ech = sqrt(pow((i-1),2)+pow((j-3),2));
            else if(b.brd[i][j]==8) ech = sqrt(pow((i-2),2)+pow((j-0),2));
...
double h3_manhattan(board b) {
    ...
    for(int i=0;i<4;i++) {
        for(int j=0;j<4;j++) {
            ...
            else if(b.brd[i][j]==3) ech = abs(i-0)+abs(j-3);
            else if(b.brd[i][j]==4) ech = abs(i-1)+abs(j-0);
            else if(b.brd[i][j]==5) ech = abs(i-1)+abs(j-1);
            else if(b.brd[i][j]==6) ech = abs(i-1)+abs(j-2);
            else if(b.brd[i][j]==7) ech = abs(i-1)+abs(j-3);
            else if(b.brd[i][j]==8) ech = abs(i-2)+abs(j-0);
...
double h4_rowcol(board b) {
    ...
    for (int i=0;i<4;i++) {
        for(int j=0;j<4;j++) {
            if(b.brd[i][j]>=4*i && b.brd[i][j]<4*(i+1) && b.brd[i][j]!=0) row++;
        }
    }
    ...
    for (int i=0;i<4;i++) {
        for(int j=0;j<4;j++) {
            if(b.brd[i][j]%4==j && b.brd[i][j]!=0) col++;
        }
    }
...
int main() {
    ...
    while(!frontier.empty()) {
        ...
        if(conversion(current[index].b)=="0123456789101112131415") break;
